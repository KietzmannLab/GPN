...
####### HCP_MMP1 data extraction

data_NSD = load_pickle('RDMs/HCP_MMP1_all_neural_rdms_correlation_special515.pkl')
hcp_rois = ['H']
print(f'Processing correlations for HCP_MMP1 ROI: HCP_MMP1')
...
for n, roi in enumerate(hcp_rois):
    
    print(f'Processing correlations for ROI: {roi}, {n+1} out of {len(hcp_rois)}')
...
with open(f'best_layers_HCP_MMP1_allrois.pkl', 'wb') as f:
    pickle.dump(save_data, f)

print(f'Saved best layers for all HCP_MMP1 ROIs to best_layers_HCP_MMP1_allrois.pkl')